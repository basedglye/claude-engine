/**
 * Integrated tour dev page (orchestrator): everything main.ts composes,
 * minus player-fps, with a posable camera and `?tick=N` to step the sim
 * before the first frame. Derived from the lane 4 dev page: decor, characters, upkeep objects and the terminal
...
if (!canvas) throw new Error("apps/hotel/dev/tour.ts: missing #app canvas");
